...
from model.pre_train_model_sceneflow.other.raft_stereo.core.raft_stereo import RAFTStereo, autocast
...
@torch.no_grad()
def validate_things(model,save_txt,total_steps,iters=16, mixed_prec=False):
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        if np.sum(nan != nan) != 0:
            logging.info(f"This is nan image,id:{val_id}")
        else:
            # epe = epe.flatten()
            # val = (valid_gt.flatten() >= 0.5) & (flow_gt.abs().flatten() < 192)
            ...
            logging.info(
                f"Validation FlyingThings epe: ({epe[val].mean().item()})")
