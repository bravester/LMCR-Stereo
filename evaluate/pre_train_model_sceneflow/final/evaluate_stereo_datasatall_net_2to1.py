...
from model.pre_train_model_sceneflow.nets.crestereo import CREStereo, autocast
...
@torch.no_grad()
def validate_eth3d(model, input_image, worklog, save_txt, tall, total_steps, infer_level=1, iters=32, split='F',
...
@torch.no_grad()
def validate_things(model,worklog,save_txt,tall,total_steps,iters=10,sum_sample=400,mixed_prec=False):
...
@torch.no_grad()
def validate_middlebury(model, input_image, worklog, save_txt, tall, total_steps, infer_level=1, iters=32, split='F',
                        mixed_prec=False,muti=False):
