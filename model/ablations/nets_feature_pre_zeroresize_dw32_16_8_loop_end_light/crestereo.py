...
class CREStereo(nn.Module):
    ...
    def forward(self, image1, image2, flow_init=None, iters=[5,3,1,4], upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            # offset :1/4
            offset_dw4 = self.conv_offset_4(fmap1)
            offset_dw4 = self.range_4 * (torch.sigmoid(offset_dw4) - 0.5) * 2.0
            ...
            # # 1/4 -> 1/32
            # # feature
            # fmap1_dw32 = F.avg_pool2d(fmap1, 8, stride=8)
            # fmap2_dw32 = F.avg_pool2d(fmap2, 8, stride=8)
            # offset_dw32 = self.conv_offset_16(fmap1_dw32)
            # offset_dw32 = self.range_16 * (torch.sigmoid(offset_dw32) - 0.5) * 2.0

            # # context
            # net_dw32 = F.avg_pool2d(net, 8, stride=8)
            # inp_dw32 = F.avg_pool2d(inp, 8, stride=8)

            # positional encoding and self-attention
            pos_encoding_fn_small = PositionEncodingSine(
                d_model=256, max_shape=(image1.shape[2] // 8, image1.shape[3] // 8)
            )
            # 'n c h w -> n (h w) c'
            x_tmp = pos_encoding_fn_small(fmap1)
            fmap1= x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])
            # 'n c h w -> n (h w) c'
            x_tmp = pos_encoding_fn_small(fmap2)
            fmap2 = x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])

            fmap1, fmap2 = self.self_att_fn(fmap1, fmap2)
            fmap1, fmap2 = [
                x.reshape(x.shape[0], image1.shape[2] // 8, -1, x.shape[2]).permute(0, 3, 1, 2)
                for x in [fmap1, fmap2]
            ]

        corr_fn = AGCL(fmap1, fmap2,att=self.cross_att_fn)
        corr_fn_dw8 = AGCL(fmap1_dw8, fmap2_dw8)
        corr_fn_att_dw16 = AGCL(fmap1_dw16, fmap2_dw16)
        ...
        for iterj in range(iters[3]):
            if (flow_init is not None) and (iterj==0):
                ...
                for itr in range(iters[0]):
                    ...
                    out_corrs = corr_fn_att_dw16(
                        flow_dw16,None, small_patch=small_patch,iter_mode=True
                        )
                ...
                for itr in range(iters[1]):
                    ...
                    out_corrs = corr_fn_dw8(flow_dw8,None, small_patch=small_patch,iter_mode=True)
            ...
            for itr in range(iters[2]):
                ...
                flow = flow.detach()
                out_corrs = corr_fn(flow,offset_dw4, small_patch=small_patch, iter_mode=False)
                flow_u=flow-coords0
                with autocast(enabled=self.mixed_precision):
