...
class CREStereo(nn.Module):
    ...
    def forward(self, image1, image2, flow_init=None, iters=[5,3,1,4], upsample=True, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision):
            ...
            # 'n c h w -> n (h w) c'
            x_tmp = pos_encoding_fn_small(fmap1_dw8)
            fmap1_dw8= x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])
            # 'n c h w -> n (h w) c'
            x_tmp = pos_encoding_fn_small(fmap2_dw8)
            fmap2_dw8 = x_tmp.permute(0, 2, 3, 1).reshape(x_tmp.shape[0], x_tmp.shape[2] * x_tmp.shape[3], x_tmp.shape[1])

            fmap1_dw8, fmap2_dw8 = self.self_att_fn(fmap1_dw8, fmap2_dw8)
            fmap1_dw8, fmap2_dw8 = [
                x.reshape(x.shape[0], image1.shape[2] // 16, -1, x.shape[2]).permute(0, 3, 1, 2)
                for x in [fmap1_dw8, fmap2_dw8]
            ]

        corr_fn = AGCL(fmap1, fmap2)
        corr_fn_dw8 = AGCL(fmap1_dw8, fmap2_dw8,att=self.cross_att_fn)
        ...
        for iterj in range(iters[3]):
            if (flow_init is not None) and (iterj==0):
                ...
                for itr in range(iters[1]):
                    ...
                    out_corrs = corr_fn_dw8(flow_dw8,offset_dw8, small_patch=small_patch,iter_mode=False)
