...
def main(args):
	...
	results_F1 = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
										 infer_level=1, iters=args.valid_iters, split='F')

	results_F2 = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
										 infer_level=2, iters=args.valid_iters, split='F')

	results_F3 = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
										 infer_level=3, iters=args.valid_iters, split='F')

	results_F4 = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
										 infer_level=4, iters=args.valid_iters, split='F')

	results_F2_True = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage, worklog=worklog,
											  save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
											  infer_level=2, iters=args.valid_iters, split='F', muti=True)
	results_F3_True = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage,
											  worklog=worklog,
											  save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
											  infer_level=3, iters=args.valid_iters, split='F', muti=True)
	results_F4_True = eva.validate_middlebury(model=model.module, input_image=args.input_size,output_image=log_outimage,
											  worklog=worklog,
											  save_txt=save_txt, tall=t0, total_steps=total_steps + 1,
	...
	results_F12 = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
										 infer_level=1, iters=args.valid_iters, split='F')

	results_F22 = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
										 infer_level=2, iters=args.valid_iters, split='F')

	results_F32 = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
										 infer_level=3, iters=args.valid_iters, split='F')

	results_F42 = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage, worklog=worklog,
										 save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
										 infer_level=4, iters=args.valid_iters, split='F')

	results_F22_True = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage, worklog=worklog,
											  save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
											  infer_level=2, iters=args.valid_iters, split='F', muti=True)
	results_F32_True = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage,
											  worklog=worklog,
											  save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
											  infer_level=3, iters=args.valid_iters, split='F', muti=True)
	results_F42_True = eva.validate_middlebury(model=model.module, input_image=args.input_size2,output_image=log_outimage,
											  worklog=worklog,
											  save_txt=save_txt2, tall=t0, total_steps=total_steps + 1,
											  infer_level=4, iters=args.valid_iters, split='F', muti=True)
...
if __name__ == '__main__':
	...
	parser.add_argument('--log_dir', default='./results_test4/light_5318_fc_mid_cres_300000',
						help='use mixed precision')
	parser.add_argument('--log_model_dir',
						default='E:\\yxz\\Deep_learning\\CREStereo_pytorch-re\\wangyu\\CREStereo-Pytorch-main-re31_1\\train\\pre_train_model_sceneflow\\final\\pre+mid+cres\\results\\gi32_16_8_light_5314_pre_mid_fc_b4_i300000_true\\checkpoints',
						help='model save pth')
	...
	parser.add_argument('--input_size3', type=int, default=[384, 512],
						help='input image size')
	parser.add_argument('--valid_iters', type=int, default=[5, 3, 1, 8],
