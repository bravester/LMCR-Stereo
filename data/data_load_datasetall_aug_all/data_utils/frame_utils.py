...
def readDispMiddlebury(file_name):
    assert basename(file_name) == 'disp0GT.pfm' or basename(file_name) == 'disp0.pfm'
...
def readDispMiddlebury_extra(file_name):
    assert basename(file_name) == 'disp0.pfm'
    disp = readPFM(file_name).astype(np.float32)
    # assert len(disp.shape) == 2
    # nocc_pix = file_name.replace('disp0.pfm', 'mask0nocc.png')
    # assert exists(nocc_pix)
    # nocc_pix = imageio.imread(nocc_pix) == 255
    # assert np.any(nocc_pix)
    return disp
