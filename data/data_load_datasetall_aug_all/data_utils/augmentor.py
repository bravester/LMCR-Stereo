...
class Augmentor:
    ...
    def __call__(self, left_img, right_img, left_disp):
        # 1. chromatic augmentation：色增强
        left_img = self.chromatic_augmentation(left_img)
        right_img = self.chromatic_augmentation(right_img)

        # 2. spatial augmentation：空间增强
        # 2.1) rotate & vertical shift for right image：右图旋转和垂直移动
        # np.random.RandomState.binomial() 从二项分布中抽取样本
        if self.rng.binomial(1, 0.5):
            angle, pixel = 0.1, 2
            # np.random.RandomState.uniform() 从均匀分布中抽取样本
        ...
        # 3. add random occlusion to right image:将随机遮挡添加到右侧图像
        if self.rng.binomial(1, 0.5):
...
class SparseAugmentor:
    ...
    def __call__(self, left_img, right_img, left_disp):
        ...
        disp_mask = (left_disp < float(self.max_disp / resize_scale)) & (np.abs(left_disp) > 0)
        disp_mask = disp_mask.astype("float32")
        disp_mask = cv2.resize(
            disp_mask,
            None,
        ...
        return left_img, right_img, left_disp, disp_mask[:,:,:1]