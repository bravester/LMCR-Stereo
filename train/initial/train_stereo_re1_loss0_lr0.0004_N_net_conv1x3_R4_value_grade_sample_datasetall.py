...
from model.cres.useful.nets_res4_conv1x1x3_R4_value_grade import Model
# from dataset import CREStereoDataset
import data.data_load_datasetall.stereo_datasets as datasets

import evaluate.evaluate_stereo_datasatall_conv1x3_R4_value_grade as eva
...
def sequence_loss_i(flow_preds,flow_gt, valid, loss_gamma, max_flow):
    ...
    # pre-process
    if flow_gt.shape!=flow_preds[0].shape:
...
def sequence_loss( flow_predictions_dw16,flow_predictions_dw8,flow_predictions_dw4, flow_gt, valid, loss_gamma=0.9, max_flow=700):
    """ Loss function defined over sequence of flow predictions """
    # 流量预测序列上定义的损失函数
    ...
    # 计算1/16的差异损失
    flow_loss_dw16,n_dw16=sequence_loss_i(flow_predictions_dw16, flow_predictions_dw8[-1], valid, loss_gamma, max_flow)
    flow_loss.append(flow_loss_dw16)

    # 计算1/8的差异损失
    flow_loss_dw8 ,n_dw8= sequence_loss_i(flow_predictions_dw8, flow_predictions_dw4[-1], valid, loss_gamma, max_flow)
    flow_loss.append(flow_loss_dw8)

    # 计算1/3的差异损失
    flow_loss_dw4,n_dw4 = sequence_loss_i(flow_predictions_dw4, flow_gt, valid, loss_gamma, max_flow)
    flow_loss.append(flow_loss_dw4)

    n_loss_sum=n_dw16+n_dw8+n_dw4
    n_flow_loss = len(flow_loss)
    loss = 0
    assert n_flow_loss >= 1
    for i in range(n_flow_loss):
        i_weight = loss_gamma ** (n_flow_loss - i - 1)
        loss += (i_weight*flow_loss[i])/n_loss_sum

    valid = valid.bool()
    epe = torch.sum((flow_predictions_dw4[-1] - flow_gt) ** 2, dim=1).sqrt()
...
def main(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(dataloader)):
            ...
            flow_predictions_16,flow_predictions_8,flow_predictions_4 = model(left, right)
            assert model.training

            # a=flow_predictions[0].shaoe

            loss, metrics = sequence_loss( flow_predictions_16,flow_predictions_8,flow_predictions_4, gt_flow, valid_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='./results/41991/train_stereo_log_cres_loss0_N_net_conv1x3_R4_value_grade_sample_datasetall_b4_i20000', help='use mixed precision')
