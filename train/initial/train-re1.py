...
def sequence_loss(flow_preds, flow_gt, valid, gamma=0.9, max_flow=700):
    ...
    flow_gt = torch.cat([flow_gt, flow_gt * 0], dim=1)  # [4, 2, 384, 512]
    for i in range(n_predictions):
        ...
        v=valid.unsqueeze(1)
        v1=v.shape
        m=(valid.unsqueeze(1) * i_loss).shape
    ...
    for epoch_idx in range(start_epoch_idx, args.n_total_epoch + 1):
        ...
        for batch_idx, mini_batch_data in enumerate(dataloader):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                # results = evaluate_stereo.validate_things(model.module, iters=args.valid_iters)
                #
                # for key in results:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='./train_log', help='use mixed precision')
    ...
    parser.add_argument('--n_total_epoch', type=int, default=400, help="batch size used during training.")
    parser.add_argument('--minibatch_per_epoch', type=int, default=300, help="batch size used during training.")
    parser.add_argument('--model_save_freq_epoch', type=int, default=20, help="batch size used during training.")
