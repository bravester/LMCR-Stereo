...
def main(args):
    ...
    # if rank == 0:
    worklog.info(f"Dataset size: {len(train_loader)}")
    ...
    for epoch_idx in range(start_epoch_idx, args.n_total_epoch + 1):
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            # optimizer.zero_grad()
            left, right, gt_disp, valid_mask = [x.cuda() for x in data_blob]
            ...
            # loss & backword
            loss = sequence_loss(
                flow_predictions,  gt_disp, valid_mask, gamma=0.9, max_flow=700
            )
