...
from model.cres.useful.nets_res4_conv1x1x3_R4_iter_grade import Model
# from dataset import CREStereoDataset
import data.data_load_datasetall.stereo_datasets as datasets

import evaluate.evaluate_stereo_datasatall_conv1x3_R4 as eva
...
def sequence_loss_i(flow_preds, flow_gt, valid, loss_gamma, max_flow):
...
def sequence_loss(flow_preds_dw16,flow_preds_dw8,flow_preds_dw4,flow_preds_dw1,
    flow_gt, valid, loss_gamma=0.9, max_flow=700):
    """ Loss function defined over sequence of flow predictions """
    # 分别计算各级损失
    flow_loss_grads=[]
    # 1/16
    flow_loss_dw16=sequence_loss_i(flow_preds_dw16, flow_gt, valid, loss_gamma, max_flow)
    flow_loss_grads.append(flow_loss_dw16)
    # 1/8
    flow_loss_dw8 = sequence_loss_i(flow_preds_dw8, flow_gt, valid, loss_gamma, max_flow)
    flow_loss_grads.append(flow_loss_dw8)
    # 1/4
    flow_loss_dw4 = sequence_loss_i(flow_preds_dw4, flow_gt, valid, loss_gamma, max_flow)
    flow_loss_grads.append(flow_loss_dw4)
    # 1/1
    flow_loss_dw1 = sequence_loss_i(flow_preds_dw1, flow_gt, valid, loss_gamma, max_flow)
    flow_loss_grads.append(flow_loss_dw1)

    n_loss_grads = len(flow_loss_grads)
    assert n_loss_grads >= 1

    flow_loss=0
    for i in range(n_loss_grads):
        i_weight = loss_gamma ** (n_loss_grads - i - 1)
        flow_loss+=(i_weight *flow_loss_grads[i])/n_loss_grads

    valid =valid.bool()
    epe = torch.sum((flow_preds_dw1[-1] - flow_gt) ** 2, dim=1).sqrt()
...
def main(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(dataloader)):
            ...
            flow_predictions_dw16,flow_predictions_dw8,flow_predictions_dw4,flow_predictions_dw1= model(left, right)
            assert model.training

            # a=flow_predictions[0].shaoe

            loss, metrics = sequence_loss(flow_predictions_dw16, flow_predictions_dw8,flow_predictions_dw4,flow_predictions_dw1, gt_flow, valid_mask)
            ...
            # # 梯度累加
            # if global_batch_num % args.accumulation_steps == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='./results/41991/train_stereo_log_cres_loss0_lr0.0004_N_net_conv1x3_R4_sample_datasetall_b4_i20000', help='use mixed precision')
    ...
    parser.add_argument('--accumulation_steps', default=4,
                        help='batch size disguised expansion accumulation_steps times')
