...
from model.ablations.nets_feature_pre_zeroresize_dw32_16_8_loop_end_light import Model
from model.ablations.nets_feature_pre_zeroresize_dw32_16_8_loop_end_light.crestereo import autocast

import data.data_load_datasetall_aug_all.stereo_datasets as datasets
import inference.inference_model as eva

# import evaluate.ablations.evaluate_stereo_datasatall_feature_2to1_add_pre_zeroresize_grade_dw32_16_8_loop_end_light_aug_all as eva
# import evaluate.L37671_211.evaluate_stereo_datasatall_2to1_add_pre_zeroresize_grade_dw32_16_8_loop_end as eva
...
def sequence_loss(flow_preds, flow_gt, valid, loss_gamma=0.9, max_flow=700):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += (i_weight * (i_loss[valid.bool()]).mean()) / n_predictions
...
def main(args):
    ...
    # datasets
    dataset = datasets.fetch_dataloader(args,worklog=worklog)
    # if rank == 0:
    ...
    # chk_path = os.path.join(log_model_dir, "CREStereo.pth")
    ...
    validation_frequency_test=200
    max_epoch = args.num_steps // len(dataloader) + 1
    worklog.info(f"max_epoch: {max_epoch}")
    worklog.info(f"train_iters: {args.train_iters}")
    worklog.info(f"valid_iters: {args.valid_iters}")
    worklog.info(f"log_dir: {args.log_dir}")
    worklog.info(f"lr: {args.lr}")
    worklog.info(f"validation_frequency: {validation_frequency}")
    worklog.info(f"validation_frequency_test: {validation_frequency_test}")
    worklog.info(f"train_datasets: {args.train_datasets}")
    ...
    while should_keep_training:
        for i_batch, (_, *data_blob) in enumerate(tqdm(dataloader)):
            ...
            meta_info.append('image_size:{}'.format(left.shape))
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                # results = eva.validate_things(model.module,worklog=worklog, save_txt=save_txt, tall= t0,total_steps=total_steps + 1,
                #                                                   iters=args.valid_iters)
                results=eva.inference(autocast=autocast,model=model.module,datasets=datasets,input_image=args.input_size,worklog=worklog,save_txt= save_txt,tall=t0,
                                      total_steps=total_steps+1,infer_level=1,iters=args.valid_iters,sum_sample=400,data_choice="sceneflow")
            ...
            if total_steps % validation_frequency_test == validation_frequency_test - 1:
                # save_path = Path(log_checkpoints_dir + '/%d_%s.pth' % (total_steps + 1, args.name))
                # worklog.info(f"Model params saved: {save_path.absolute()}")
                # torch.save(model.state_dict(), save_path)

                save_txt = args.log_dir + '/' + args.save_txt+'_test_sum_sample20'
                results2 = eva.inference(autocast=autocast, model=model.module, datasets=datasets, input_image=args.input_size,
                                        worklog=worklog, save_txt=save_txt, tall=t0,
                                        total_steps=total_steps + 1, infer_level=1, iters=args.valid_iters, sum_sample=20,data_choice="sceneflow")

                logger.write_dict(results2,tb_log)
                # logger.writer.add_scalar("epe_test", results2["epe"], global_batch_num)
                # logger.writer.add_scalar("d1_test", results2["d1"], global_batch_num)
                # logger.writer.add_scalar("d3_test", results2["d3"], global_batch_num)
                # logger.writer.add_scalar("d5_test", results2["d5"], global_batch_num)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='./results/zero_gi32_16_8_light_3225_loop_end_allfc_b4_i40000_true', help='use mixed precision')
    parser.add_argument('--log_model_dir',
                        default='./results/train_stereo_log_cres_loss_N_net_2to1_sample_datasetall_b4_i100000/checkpoints',
                        help='model save pth')
    parser.add_argument('--save_txt', default='FlyingThings3D (TEST)', help='use mixed precision')
    ...
    # sceneflow_f+c，sceneflow
    parser.add_argument('--train_datasets', nargs='+', default=['sceneflow_f+c'], help="training datasets.")
    ...
    # [5,3.1,4] ==>[20,12,4,1]
    parser.add_argument('--train_iters', type=int, default=[3,2,2,5],
                        help="number of updates to the disparity field in each forward pass.")
    # sceneflow :[544,960] ,middlebury ;[1536.2048],kitti2015: [320,512]或者[320,1024]
    parser.add_argument('--input_size', type=int, default=[544,960],
                        help='input image size')
    parser.add_argument('--wdecay', type=float, default=.00001, help="Weight decay in optimizer.")

    # Validation parameters
    parser.add_argument('--valid_iters', type=int, default=[3,2,2,5],
